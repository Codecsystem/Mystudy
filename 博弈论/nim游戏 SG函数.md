...
- 若初态为**必胜态**（$a_1 \oplus a_2 \oplus \cdots \oplus a_n \neq 0$），则**先手必胜**；
- 若初态为**必败态**（$a_1 \oplus a_2 \oplus \cdots \oplus a_n = 0$），则**先手必败**。

##### 定理证明

**定理 1：必胜态的后继状态至少存在一个必败态**

证明：

设 $a_1 \oplus \cdots \oplus a_n = s \neq 0$，设 $s$ 的二进制位为 $1$ 的最高位是第 $k$ 位，则 $a_1, \cdots, a_n$ 中一定有奇数个 $a_i$ 的二进制的第 $k$ 位为 $1$。

用 $a_i \oplus s$ 去替换 $a_i$，则：

$$
\begin{aligned}
a_1 \oplus \cdots \oplus (a_i \oplus s) \oplus \cdots \oplus a_n
&= a_1 \oplus \cdots \oplus a_i \oplus s \oplus \cdots \oplus a_n \\
&= s \oplus s \\
&= 0
\end{aligned}
...
同时，因为 $a_i \oplus s < a_i$，是合法的替换。

**定理 2：必败态的后继状态均为必胜态**

证明：

设 $a_1 \oplus \cdots \oplus a_n = 0$，则相同位上 $1$ 的个数为偶数。此时，无论减少哪个数，都会使得异或和 $\neq 0$。

必胜态与必败态交替出现，终态 $(0,0,\dots,0)$ 是必败态。

递推边界：

- 所有堆都为 $0$ 时，异或和为 $0$，必败态。
- 若当前局面的异或和不为 $0$，则一定能操作到异或和为 $0$ 的局面，是必胜态。
- 若当前局面的异或和为 $0$，则任意一次合法操作后异或和都会变为非 $0$，是必败态。
...
\begin{aligned}
X'
&= SG(s_1) \oplus \cdots \oplus SG(t) \oplus \cdots \oplus SG(s_n) \\
