...
/*
朴素dijk核心步骤
初始化：dis 数组全设为无穷大，dis[S] = 0，vis 数组全为 0。
迭代 $V$ 次（外层循环）：找最小值：遍历所有节点 $1 \dots V$，在所有 vis[i] == 0 的点中，找到 dis[i] 最小的节点 $u$。
标记：将 $u$ 标记为已确定最短路，即 vis[u] = 1。
松弛：遍历 $u$ 的所有出边（如果是邻接矩阵，则遍历所有节点 $v$），若 dis[v] > dis[u] + w(u, v)，则更新 dis[v]。
*/