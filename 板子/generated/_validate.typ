...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[两者是双射]
...
具体来说 考虑dp[i-2] dp[i-1]的含义 然后再考虑组合意义
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[被傻逼abc e题击败了 遂记录之]

====== 0.一些概念

*1).团* ：一个完全子图
*2).最大独立集* ：一个点数最多的顶点集，其中任意两个顶点之间都没有边
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[可以发现这两是对偶关系]

*3).最小染色*：用最少的颜色给点染色使得所有边连接的两点颜色不同。
*4).色数*：最小染色中使用的颜色数
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[其实下面所述都属于完美图，即最大团=最小染色数]
...
两元素之间可比，则存在一条无向边
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[通常我们会画成有向图（如果定义了偏序关系），但本质是无向的（只关心可比关系）]

*链*：构成一条链的元素两两之间可比 $arrow.r.double$ 对应图中的 团 。
*反链*：构成一条反链的元素两两之间不可比 $arrow.r.double$ 对应图中的 独立集 。
*链划分*：将一个偏序集划分成若干条链
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[其实此处的最长链是好求的，就是DAG的最长路]
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[最大反链也是好求的，跑个传递闭包，按二分图的方法跑最小链覆盖就行]

#image("E:/Data/Work/algorithm_study/Mystudy/Graph/md.img/image.png", width: 90%)

#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[这是个整除图来着，红色的是链 蓝色的是最大反链 注意到最小链划分数是2，最大反链的元素数目也是2]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[所以我们看到可比图的时候，先想想这个可比图的链是啥含义，反链又是啥含义，这样有可能把一般可比图的求最大反链的O(n^1/2*m)优化成O(nlogn)]

====== 2.二分图

#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[详细证明见二分图最大匹配.cpp 注释部分]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[其实对于一般图来说，最大独立集也=n-最小点覆盖中的顶点数量]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[也就是说 对于一般的dag我们可以通过这个转换 最大流O(n^1/2*m)求出最小链划分]
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[注意路径是不交的 如果要相交先对原图跑一个传递闭包]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[注意到区间图没有长度 $g e 4$ 的无弦环，所以他是弦图]
随便画画图就能证明

======= 1).最大团：直线上重叠层数最多的位置 \<-\> 最小染色:给每个区间染色使得重叠区间颜色不同，最少颜色数

#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[此处的最大团=最小染色比较显然]

经典模型：求区间图最大团
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[把区间变成{l,+1},{r,-1},然后对坐标排序,注意相同的时候+1放前面即可，做扫描线即可]

经典模型：求区间最小染色的方案
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[建立小根堆，里面存{某颜色的结束时间, 颜色ID}，每次取出堆顶，如果堆顶的结束时间小于当前区间的开始时间，则该区间可以染该颜色，否则该区间无法染色，需要新开一个颜色，然后加入堆中]

======= 2).最大独立集：求最多的不交区间数

#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[这是经典的，按右端点 $R$ 从小到大排序，选了 $i$ 后，下一个选 $L_j > R_i$ 的第一个 $j$。]

当然当我们要求最小点覆盖的时候只要用n减去最大独立集即可

======= 3).选最少的点，使得每个区间内至少包含一个点。

#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[经典贪心：按右端点 $R$ 从小到大排序。如果当前区间不交，贪心地选该区间的最右端点（因为它能尽可能多地向右覆盖后续区间的左端点）。]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[按l排序，维护当前已覆盖到的右边界r，在当前区间中找最大的r即可]

每个区间 $[L_i, R_i]$ 有权值 $W_i$，求互不相交的区间集合使得权值和最大
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[随便dp即可]

====== 4.弦图

*弦图*：没有大于 3 的“无弦环”的图，即他没有大洞
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[树也是弦图！]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[感性理解一下，每次选相连边数最多的那个点可以保证团的纯度]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[随便归纳一下就行，类似于n循环比赛]
...
#block(inset: (left: 12pt), stroke: (left: 2pt + luma(180)))[感性理解，考虑交换论证，设u，v,u的出现早于v，u的团包含v，考虑选u，团内只能选一个，不劣，考虑选v，扼杀了v构成的团内的某些可能，劣。]
...
- 若初态为*必胜态*（$a_1 and.big a_2 and.big dots.c and.big a_n eq.not 0$），则*先手必胜*；
- 若初态为*必败态*（$a_1 and.big a_2 and.big dots.c and.big a_n = 0$），则*先手必败*。
...
*证明*：  
设 $a_1 and.big dots.c and.big a_n = s eq.not 0$，设 $s$ 的二进制位为 1 的最高位是第 $k$ 位，则 $a_1, dots.c, a_n$ 中一定有奇数个 $a_i$ 的二进制的第 $k$ 位为 1。  
用 $a_i and.big s$ 去替换 $a_i$，则：


$a_1 and.big dots.c and.big (a_i and.big s) and.big dots.c and.big a_n = a_1 and.big dots.c and.big a_i and.big s and.big dots.c and.big a_n = s and.big s = 0$


这是一种必败态。  
同时，因为 $a_i and.big s < a_i$，是合法的替换。
...
*证明*：  
设 $a_1 and.big dots.c and.big a_n = 0$，则相同位上 1 的个数为偶数。此时，无论减少哪个数，都会使得异或和 ≠ 0。  

必胜态与必败态交替出现，终态 $(0,0,dots,0)$ 是必败态。

终态：$a_i = 0$，异或和为 0，必败态。
      $a_i eq.not 0$，异或和 ≠ 0，必胜态。


=== 台阶型 Nim游戏


有 \$1 tilde n\$ 级台阶，第 \$i\$ 级台阶上摆放 \$a_i\$ 个石子，每次操作可将第 \$k\$ 级台阶上的石子移一些到第 \$k-1\$ 级台阶上，移到第 \$0\$ 级台阶（地面）的石子不能再移动。
...
若奇数台阶上的石子数异或和不为 0，即 $a_1 xor a_3 xor a_5 xor dots.c eq.not 0$，则先手必胜；否则先手必败。
...
$upright("m e x")(S) = min\{x\} quad (x in N, x in.not S)$
...
设状态（节点）$x$ 有 $k$ 个后继状态（子节点）$y_1, y_2, dots.c, y_k$，
...
由 $n$ 个有向图游戏组成的组合游戏，设起点分别为 $s_1, s_2, dots.c, s_n$，当
...
- 因此 $X = 0 xor 0 xor dots.c xor 0 = 0$
- 无法进行任何操作，当前玩家失败

====== 2. 若 $X eq.not 0$，存在操作使 $X = 0$

设 $X = k eq.not 0$，则存在某个 `\( SG(s_i) \)` 使得 `\( SG(s_i) \oplus k \< SG(s_i) \)`
...
4.最优交换次数 $K$：将排列 $P$ 分解为若干个不相交的置换环。设环的个数为 $C$，则将 $P$ 排序为 $(1, 2, dots, N)$ 的最小交换次数为：$K = N - C$
