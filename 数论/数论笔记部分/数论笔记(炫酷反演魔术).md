...
其中 $\mathbf{Z}$ 是一个变换矩阵。
所谓的“反演”，就是求逆矩阵 $\mathbf{M} = \mathbf{Z}^{-1}$：
...
$$
\text{Sum} = \sum_{S \le (u, v) \le E} \mu_P(x_1, u) \cdot \mu_Q(y_1, v)
$$

因为积偏序集中，$u$ 和 $v$ 的选取是独立的（$x_1 \le u \le x_2$ 且 $y_1 \le v \le y_2$），我们可以把双重求和拆成两个括号相乘：

$$
\text{Sum} = \left( \sum_{x_1 \le u \le x_2} \mu_P(x_1, u) \right) \times \left( \sum_{y_1 \le v \le y_2} \mu_Q(y_1, v) \right)
$$
...
$$
E(\max_{i \in S}{}_k x_i) = \sum_{T \subseteq S,T \ne \emptyset} (-1)^{|T|-k} \binom{|T|-1}{k-1} E(\min_{j \in T} x_j)
$$

$$
E(\min_{i \in S}{}_k x_i) = \sum_{T \subseteq S,T \ne \emptyset} (-1)^{|T|-k} \binom{|T|-1}{k-1} E(\max_{j \in T} x_j)
$$
